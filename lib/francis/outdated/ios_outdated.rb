...
class IOSOutdated
  ...
  def check_pods_dependencies
    ...
    if File.exist?("Podfile.lock")
      ...
      cocoapods_message = CommandLineClient.execute("pod outdated")
      ...
      plugin.message("Cocoapods: #{cocoapods_message}")
    end
  ...
  def check_carthage_dependencies
    ...
    if File.exist?("Cartfile.resolved")
      ...
      carthage_message = CommandLineClient.execute("carthage outdated")
      ...
      plugin.message("Carthage: #{carthage_message}")
    end
    return { total: total_dependencies_count, outdated: outdated_dependencies_count }
  end
  ...
  private :check_carthage_dependencies, :check_pods_dependencies
end
